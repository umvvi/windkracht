        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin-bottom: 2rem;">
            ...
            <div style="background: white; border-radius: 0.5rem; box-shadow: 0 2px 12px rgba(0,0,0,0.08); overflow: hidden; border-left: 4px solid #dc2626;">
                <div style="background: #fef2f2; padding: 1.5rem; border-bottom: 1px solid #fee2e2;">
                    ...
                    <p style="font-size: 1.3rem; font-weight: 800; color: #dc2626; margin: 0;">{{ $lesson->start_time->format('d-m-Y') }}</p>
                    <p style="font-size: 2rem; font-weight: 800; color: #dc2626; margin: 0.25rem 0 0 0;">{{ $lesson->start_time->format('H:i') }}</p>
                </div>
                <div style="padding: 1.5rem;">
                    <div style="margin-bottom: 1rem;">
                        <p style="color: #9ca3af; font-size: 0.8rem; font-weight: 600; margin: 0 0 0.5rem 0;">PAKKET</p>
                        <p style="color: #1f2937; font-weight: 600; margin: 0; font-size: 1rem;">{{ $reservation->package->name }}</p>
                    </div>
                    <div style="margin-bottom: 1rem;">
                        <p style="color: #9ca3af; font-size: 0.8rem; font-weight: 600; margin: 0 0 0.5rem 0;">LOCATIE</p>
                        <p style="color: #1f2937; font-weight: 600; margin: 0; font-size: 1rem;">📍 {{ $reservation->location->name }}</p>
                    </div>
                    <div>
                        <p style="color: #9ca3af; font-size: 0.8rem; font-weight: 600; margin: 0 0 0.5rem 0;">RESTERENDE LESSEN</p>
                        <p style="color: #10b981; font-weight: 800; margin: 0; font-size: 1.2rem;">{{ $remainingSessions }} {{ $remainingSessions === 1 ? 'les' : 'lessen' }}</p>
                    </div>
                </div>
            </div>
                ...
                <div style="padding: 1.5rem;">
                    <form action="{{ route('customer.reschedule-lesson.store', $lesson->id) }}" method="POST" id="rescheduleForm">
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            >
                            @error('new_date')
                                <span style="color: #dc2626; font-size: 0.85rem; margin-top: 0.5rem; display: block; font-weight: 500;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div style="display: flex; gap: 1rem;">
                            <a href="{{ route('customer.reservations') }}" style="flex: 1; padding: 0.85rem 1rem; border: 2px solid #e5e7eb; border-radius: 0.4rem; background: white; color: #1f2937; font-weight: 600; text-decoration: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.3s; text-align: center;" onmouseover="this.style.background='#f9fafb'; this.style.borderColor='#d1d5db'" onmouseout="this.style.background='white'; this.style.borderColor='#e5e7eb'">
                                ← Terug
                            </a>
                            <button 
                                type="submit" 
